...
print('This program solves de KdV equation using two finite difference schemes')
...
if (scheme==1):
    ...
    for m in range (1,p-1) :
        cuenta+=0.001
        posicion=int(cuenta)
        if (True in (u[:,m]>1+c/2)):
            break
        for j in range(posicion,2*solint+posicion):
            u[j,m+1]=u[j,m-1]-2*(dt/dx)*(u[j+1,m]+u[j,m]+u[j-1,m])*(u[j+1,m]-u[j-1,m])-dt/(dx**3)*(u[j+2,m]-2*u[j+1,m]+2*u[j-1,m]-u[j-2,m])
        k+=1
    ...
    for m in range (p-1) :
        ...
        if (True in (u[:,m]>c/2)):
            break
        for j in range(posicion,2*solint+posicion):
            u[j,m+1]=u[j,m]-(dt/dx)*( Phi(u[j,m])-Phi(u[j-1,m]) )-(dt/(2*(dx**3)))*(u[j+2,m]-2*u[j+1,m]+2*u[j-1,m]-u[j-2,m])
        k+=1
...
dui=np.diff(u[:,0])/dx
dui=np.append([0.01],dui)
dui2=np.square(dui)

dum=np.diff(u[:,int(k/2)])/dx
dum=np.append([0.01],dum)
dum2=np.square(dum)

duf=np.diff(u[:,0])/dx
duf=np.append([0.01],duf)
duf2=np.square(duf)

integrandi=u3i-dui2/2
integrandm=u3m-dum2/2
integrandf=u3f-duf2/2
